...
class LCD(threading.Thread):
    ...
    def __init__(self, port, baud, size=(128, 64),
        ...
        # Max data controller can buffer
        self.__buffer = Queue.Queue(self.buffer_size)
        self.__stop = False
    ...
    def run(self):
        ...
        while True:
            ...
            if next_beat > 0:
                log.debug(
                    "Waiting {0} seconds for next heartbeat".format(next_beat))
    ...
    def __set_pos_x(self, x):
        if x < 0 or x > self.size[0]:
            raise ValueError(
                "X coordinate must be in range 0-{0}".format(self.size[0]))
        self.send("|\x18" + chr(x))

    def __set_pos_y(self, y):
        if y < 0 or y > self.size[1]:
            raise ValueError(
                "Y coordinate must be in range 0-{0}".format(self.size[1]))
        self.send("|\x19" + chr(y))
    ...
    def line(self, x1, y1, x2, y2, draw=True):
        ...
        (Pixel positions start at the bottom left of the screen)
        """
        self.send("|\x0c" + chr(x1) + chr(y1)
                  + chr(x2) + chr(y2) + chr(int(draw)))
